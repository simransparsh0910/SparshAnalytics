...
def load_license_file(file_path='license.bin'):
...
def save_detected_face(cropped_face, name):
    ...
    if not os.path.exists('detected_faces'):
        os.makedirs(f'detected_faces')
    
    today_date = datetime.now().strftime('%Y-%m-%d')
    date_folder_path = os.path.join(f'detected_faces',today_date)
...
def find_original_image(person_name):
    folder_path = f"faces/{person_name}/"
...
def save_received_face_image(person_name, row_id, image_base64):
    folder_name = f"{person_name}_{row_id}"
    folder_path = f"faces/{folder_name}/"
    if not os.path.exists(folder_path):
        os.makedirs(folder_path)
...
def process_stream(rtsp_id, rtsp_url,roi, detector, recognizer, embedding_manager, zmq_socket,zmq_socket_send_data,stream_flags, max_retries=-1, reconnect_delay=5,full_frames_folder=f"FullFrames"):
    retries = 0
...
def main():
    try:
        ...
        recognizer = ArcFace(model_path="face_matcher.onnx")
