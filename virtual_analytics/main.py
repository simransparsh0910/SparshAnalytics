...
model = YOLO('/app/virtual_analytics/yolov8s.pt')
model.to(device)

csrnet_model_path = "/app/virtual_analytics/CSRNET_PartAmodel_best.pth"
csrnet_model = load_csrnet_model(csrnet_model_path,device)

pose_model = YOLO("/app/virtual_analytics/yolov8s-pose.pt").to(device)

person_attributes_model_path = "/app/virtual_analytics/Person_attributes_resnet50_peta.pth"
person_attributes_model = load_person_attributes_resnet_model(person_attributes_model_path,device)

fire_smoke_model = YOLO("/app/virtual_analytics/fireandsmoke.pt")
fire_smoke_model.to(device)

fall_detection_model = YOLO("/app/virtual_analytics/fall_model.pt").to(device)
...
if not os.path.exists('/app/virtual_analytics/LOGS'):
    os.makedirs('/app/virtual_analytics/LOGS')
...
def configure_logging(rtsp_id):
    ...
    if not logger.handlers:
        fh = logging.FileHandler(f'/app/virtual_analytics/LOGS/{rtsp_id}.log')
...
def save_frame_and_send_intrusion_alert(rtsp_id, frame, person_id, folder_path='/app/virtual_analytics/intrusion', logger=None):
    try:
...
def save_frame_and_send_intrusion_attribute_alert(rtsp_id, frame, person_id, additional_data, folder_path='/app/virtual_analytics/intrusion_attributes' , logger=None):
    try:
        today_date = datetime.now().strftime("%Y%m%d")
        date_folder_path = os.path.join(folder_path, today_date)
        if not os.path.exists(date_folder_path):
...
def save_frame_and_send_loitering_alert(rtsp_id, frame, person_id, folder_path='/app/virtual_analytics/loitering', logger=None):
    try:
        today_date = datetime.now().strftime("%Y%m%d")
        date_folder_path = os.path.join(folder_path, today_date)
        if not os.path.exists(date_folder_path):
...
def save_frame_and_send_crowd_formation_alert(rtsp_id, frame, count, folder_path='/app/virtual_analytics/crowdformation_alerts', logger=None):
    try:
        today_date = datetime.now().strftime("%Y%m%d")
        date_folder_path = os.path.join(folder_path, today_date)
        if not os.path.exists(date_folder_path):
...
def save_frame_and_send_crowd_estimation_alert(rtsp_id, frame, count,density_map_resized, folder_path='/app/virtual_analytics/crowdestimation_alerts', logger=None):
    try:
        today_date = datetime.now().strftime("%Y%m%d")
        date_folder_path = os.path.join(folder_path, today_date)
...
def save_frame_and_send_crowd_dispersion_alert(rtsp_id, frame, count, folder_path='/app/virtual_analytics/crowd_dispersion_alerts', logger=None):
    try:
        today_date = datetime.now().strftime("%Y%m%d")
        date_folder_path = os.path.join(folder_path, today_date)
...
def save_frame_and_send_fire_smoke_alert(rtsp_id, frame, detection_type, folder_path='/app/virtual_analytics/fire_smoke_alerts', logger=None):
    try:
        today_date = datetime.now().strftime("%Y%m%d")
        date_folder_path = os.path.join(folder_path, today_date)
        if not os.path.exists(date_folder_path):
...
def save_frame_and_send_waving_alert(rtsp_id, frame, person_id, folder_path='/app/virtual_analytics/waving_alerts', logger=None):
    try:
        today_date = datetime.now().strftime("%Y%m%d")
        date_folder_path = os.path.join(folder_path, today_date)
        if not os.path.exists(date_folder_path):
...
def save_frame_and_send_fall_alert(rtsp_id, frame, detection_type, folder_path='/app/virtual_analytics/fall_alerts', logger=None):
    try:
        today_date = datetime.now().strftime("%Y%m%d")
        date_folder_path = os.path.join(folder_path, today_date)
        if not os.path.exists(date_folder_path):
...
def save_frame_and_send_wrong_direction_alert(rtsp_id, frame, person_id, direction, folder_path='/app/virtual_analytics/wrong_direction_alerts', logger=None):
    try:
        today_date = datetime.now().strftime("%Y%m%d")
        date_folder_path = os.path.join(folder_path, today_date)
        if not os.path.exists(date_folder_path):
...
def save_frame_and_send_waiting_time_alert(rtsp_id, frame, person_id, waiting_time, folder_path='/app/virtual_analytics/waiting_time_alerts', logger=None):
    try:
        today_date = datetime.now().strftime("%Y%m%d")
        date_folder_path = os.path.join(folder_path, today_date)
        if not os.path.exists(date_folder_path):
...
def save_frame_and_send_directional_arrow_alert(rtsp_id, frame, person_id, direction, folder_path='/app/virtual_analytics/directional_arrow_alerts', logger=None):
    try:
        today_date = datetime.now().strftime("%Y%m%d")
        date_folder_path = os.path.join(folder_path, today_date)
        if not os.path.exists(date_folder_path):
...
def send_in_out_alert(rtsp_id, frame, entry_count, exit_count, folder_path='/app/virtual_analytics/in_out_alerts', logger=None):
    try:
        today_date = datetime.now().strftime("%Y%m%d")
        date_folder_path = os.path.join(folder_path, today_date)
        if not os.path.exists(date_folder_path):
                ...
                if action == "add_device":
                    for data in data_list:
                        ...
                        if rtsp_id in streams:
                            ...
                            save_streams_to_json('/app/virtual_analytics/streams.json', streams, analytics_dict,stream_metadata)
...
def main():
    try:
        streams, analytics_dict, stream_metadata = load_streams_from_json('/app/virtual_analytics/streams.json')
